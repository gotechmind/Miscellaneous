...
public class PawnTest {
	...
	public void testGetPossibleCells() {
		List<Cell> cells = new Pawn().getPossibleCells(3, new Cell(1,1));
		assertEquals(cells.size(), 3);
		
		assertEquals(2, cells.get(0).getRow());
		assertEquals(2, cells.get(0).getColumn());
		
		assertEquals(2, cells.get(1).getRow());
		assertEquals(1, cells.get(1).getColumn());
		
		assertEquals(2, cells.get(2).getRow());
		assertEquals(0, cells.get(2).getColumn());
